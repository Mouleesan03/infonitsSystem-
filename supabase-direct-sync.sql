...
alter table public.renewals add column if not exists expiry_date date;
...
alter table public.notifications add column if not exists payload jsonb not null default '{}';
...
create policy "Current app can sync users" on public.app_users for all using (true) with check (true);
